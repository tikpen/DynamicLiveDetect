            ...
            if (self.recordEncoder.writer.status == AVAssetWriterStatusUnknown) {
                ...
                [self.recordEncoder finishWithCompletionHandler:^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter] postNotificationName:@"uploadVideoData" object:path userInfo:[NSDictionary dictionaryWithObject:path forKey:@"videoPath"]];
...
{
    if(error != NULL){
        NSLog(@"%@",NSLocalizedString(@"保存图片失败", nil));
    }else{
        NSLog(@"%@",NSLocalizedString(@"图片已保存到相册", nil));
    }
}
...
{
    if(error != NULL){
        NSLog(@"%@",NSLocalizedString(@"保存视频失败", nil));
    }else{
        NSLog(@"%@",NSLocalizedString(@"保视频已保存到相册", nil));
    }
}
